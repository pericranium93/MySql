-- Определяем кто больше поставил лайков (всего)
...
-- Выводим для каждого пользователя количество созданных сообщений, постов, загруженных медиафайлов и поставленных лайков.
select 
	users.id, 
...
-- Подсчитываем количество лайков которые получили 10 самых молодых пользователей.
select 
	users.id,
	concat_ws(' ', users.first_name, users.last_name) as name,
